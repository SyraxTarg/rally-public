...
export async function fetchApi(route: string, method: string = "GET", body?: any, cache: string = "no-store") {
  try {
...
export async function fetchSignaledCommentsApi(filters = {}) {
  const filterKeyMap = {
    comment_reason_id: "reason_id",
    commentDate: "date",
    comment_signaled_by_user: "email_user",
    email_comment_signaled: "email_comment_user",
  };
  ...
  return await fetchApi(`/signaledComments?${query}`);
}
...
export async function fetchEventsApi(offset: number, limit: number) {
  return await fetchApi(`/events?offset=${offset}&limit=${limit}`)
}
